...
//slices
import { useCreateOrderMutation } from "@/app/store/api/paymentApi";
import { useGetBookingStatusQuery } from "@/app/store/api/bookingApi";
...
const Checkout = () => {
  ...
  const { data: bookingData } = useGetBookingStatusQuery(
    { id: String(bookingId) },
    {
      skip: !bookingId,
  ...
  useEffect(() => {
    if (
      bookingData?.booking?.[0]?.bookingStatus === "success" &&
      bookingData?.booking?.[0]?.paymentStatus === "success"
    ) {
      const { event, bookingId } = bookingData?.booking?.[0] || {};
      router.replace(
        `/success?title=${event?.title}&location=${event?.location}&date=${event?.date}&bookingId=${bookingId}`,
      );
    }
  }, [bookingData]);
  return (
